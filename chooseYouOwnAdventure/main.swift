...
import Foundation
print("So i think I have WAY to much time, in most part due to working through breaks so ima make another game")
print("1 for the game i was meant to make and 2 for my boredom")
let pickGame = readLine()
if let pickGame = pickGame{
    if pickGame.contains("1"){
    ...
    if (pickGame.contains("2")){
        print("A Clasic game of Reach 21")
        print("Starting a 0, increase by 1, 2, 3, 4, whoever says 21 wins")
        var currentValue = 0
        while currentValue < 21 {
            print("Current value is \(currentValue)")
            let enterNum = readLine()
            if let enterNum = enterNum{
                if (Int(enterNum)! <= 4) && (Int(enterNum)! >= 1){
                    print("\(currentValue) + \(enterNum)")
                    currentValue += Int(enterNum)!
